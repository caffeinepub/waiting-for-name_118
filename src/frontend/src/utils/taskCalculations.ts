...
import { format, startOfWeek, endOfWeek, eachDayOfInterval, isWithinInterval, differenceInDays, startOfMonth, endOfMonth } from "date-fns";
...
export type ProductivityLevel = "beginner" | "consistent" | "dedicated" | "elite";
...
export function calculateWeeklySummary(tasks: Task[], referenceDate: Date = new Date()): WeeklySummary {
  ...
  const totalCompletionRate = validDays.length > 0
...
export function calculateCategoryScores(tasks: Task[], date: string): CategoryScore[] {
  const dateTasks = tasks.filter((t) => t.date === date);
...
export function calculateStreaks(tasks: Task[]): StreakInfo {
  ...
  const categoryStreaks = categories.map((category) => {
    ...
    for (let i = 0; i < sortedDates.length; i++) {
      const date = sortedDates[i];
      const categoryTasks = tasks.filter((t) => t.date === date && t.category === category);
...
export function calculateAchievements(tasks: Task[]): Achievement[] {
  ...
  const hasPerfectWeek = weeklySummary.dailySummaries.every((d) => d.completionRate === 100 && d.totalTasks > 0);
  ...
  return [
    ...
    },
    {
      ...
      unlocked: streaks.currentStreak >= 7 || streaks.longestStreak >= 7,
      icon: "🔥",
    },
    {
      id: "month-master",
      ...
      unlocked: streaks.currentStreak >= 30 || streaks.longestStreak >= 30,
      icon: "👑",
    },
    {
      id: "perfect-week",
...
export function calculateProductivityLevel(tasks: Task[]): ProductivityLevelInfo {
...
// Get monthly data
export function getMonthlyData(tasks: Task[], referenceDate: Date = new Date()) {
