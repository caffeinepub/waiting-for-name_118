import { C as Category } from "./index-DNhaUe03.js";
...
function calculateDailySummary(tasks, date) {
...
function calculateWeeklySummary(tasks, referenceDate = /* @__PURE__ */ new Date()) {
  ...
  const averageScore = validDays.length > 0 ? validDays.reduce((sum, d) => sum + d.score, 0) / validDays.length : 0;
  const totalCompletionRate = validDays.length > 0 ? validDays.reduce((sum, d) => sum + d.completionRate, 0) / validDays.length : 0;
  const bestDay = validDays.length > 0 ? validDays.reduce((best, d) => d.score > best.score ? d : best) : null;
  const worstDay = validDays.length > 0 ? validDays.reduce((worst, d) => d.score < worst.score ? d : worst) : null;
  ...
  const firstHalfAvg = firstHalf.length > 0 ? firstHalf.reduce((sum, d) => sum + d.score, 0) / firstHalf.length : 0;
  const secondHalfAvg = secondHalf.length > 0 ? secondHalf.reduce((sum, d) => sum + d.score, 0) / secondHalf.length : 0;
  let trend = "stable";
...
function calculateCategoryScores(tasks, date) {
  const dateTasks = tasks.filter((t) => t.date === date);
  const categories = [
    Category.study,
    Category.fitness,
...
function calculateStreaks(tasks) {
  ...
  for (let i = 0; i < sortedDates.length; i++) {
    ...
    if (summary.completionRate >= 70) {
      ...
      } else if (i === 0 && differenceInDays(new Date(today), new Date(date)) === 1) {
        currentStreak = tempStreak;
      } else if (i > 0) {
...
function calculateProductivityLevel(tasks) {
  ...
  const progress = nextLevel ? level === "beginner" ? completedTasks / 25 * 100 : level === "consistent" ? completedTasks / 100 * 100 : completedTasks / 250 * 100 : 100;
  return {
    level,
...
function getMonthlyData(tasks, referenceDate = /* @__PURE__ */ new Date()) {
...
const taskCalculations = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  CATEGORY_LABELS,
...
export {
  ...
  eachDayOfInterval as a,
  calculateCategoryScores as b,
  calculateDailySummary as c,
  calculateStreaks as d,
  endOfWeek as e,
  format as f,
  getTodayDateString as g,
  calculateWeeklySummary as h,
  getISOWeek as i,
  startOfYear as j,
  calculateAchievements as k,
  calculateProductivityLevel as l,
  constructFrom as m,
  startOfDay as n,
  startOfMonth as o,
  endOfMonth as p,
  getMonthlyData as q,
  taskCalculations as r,
  startOfWeek as s,
  toDate as t
};
